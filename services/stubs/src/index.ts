...
const control: Record<'catalogue' | 'orders' | 'promise', Control> = {
...
app.post('/_control/:service', async (c) => {
  const service = c.req.param('service') as keyof typeof control;
...
async function gate(service: keyof typeof control) {
    ...
    orders: [
      {
        ...
        line_items: [{ sku: 'sku_0007', title: 'Product 7', qty: 1 }],
      },
    ],
  });
});
